...
public class ConfigController extends DialogFragment {
	...
	public ConfigController(Context context, boolean isNav, int style,
			int preset) {
		this.isNav = isNav;
		this.style = style;
		this.color = preset;
	...
	public ConfigControllerListener setConfigControllerListener(
	...
	public Dialog onCreateDialog(Bundle savedStateInstance) {
		...
		final Spinner spinner_predef = (Spinner) v
				.findViewById(R.id.spinner_predef);
		...
		SeekBar.OnSeekBarChangeListener seek_l = new SeekBar.OnSeekBarChangeListener() {
			...
			public void onProgressChanged(SeekBar seekBar, int progress,
				...
				if (fromUser) {
					...
					color = Color.argb(seek_a.getProgress(),
							seek_r.getProgress(), seek_g.getProgress(),
							seek_b.getProgress());
				}
				edit_hex.setText(Statics.getColorHexString(color));
				seek_a.setProgressDrawable(new GradientDrawable(
						GradientDrawable.Orientation.LEFT_RIGHT,
						new int[] {
								Color.argb(0, Color.red(color),
										Color.green(color), Color.blue(color)),
								Color.argb(255, Color.red(color),
										Color.green(color), Color.blue(color)) }));
				seek_r.setProgressDrawable(new GradientDrawable(
						GradientDrawable.Orientation.LEFT_RIGHT,
						new int[] {
								Color.argb(Color.alpha(color), 0,
										Color.green(color), Color.blue(color)),
								Color.argb(Color.alpha(color), 255,
										Color.green(color), Color.blue(color)) }));
				seek_g.setProgressDrawable(new GradientDrawable(
						GradientDrawable.Orientation.LEFT_RIGHT,
						new int[] {
								Color.argb(Color.alpha(color),
										Color.red(color), 0, Color.blue(color)),
								Color.argb(Color.alpha(color),
										Color.red(color), 255,
										Color.blue(color)) }));
				seek_b.setProgressDrawable(new GradientDrawable(
						GradientDrawable.Orientation.LEFT_RIGHT,
						new int[] {
								Color.argb(Color.alpha(color),
										Color.red(color), Color.green(color), 0),
								Color.argb(Color.alpha(color),
		...
		edit_hex.addTextChangedListener(new TextWatcher() {
			...
			public void afterTextChanged(Editable s) {
				if (s.toString().startsWith("#") && s.length() == 9
						|| s.length() == 8) {
	...
	private void updateViews(View... views) {
		for (int i = 0; i < views.length; i++) {
			views[i].setBackground(Statics.getDrawable(style, isNav, color,
